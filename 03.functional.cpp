...
template <typename T> struct Generator {
    ...
    template <typename F> std::invoke_result_t<F, T> flat_map(F f) {
        while (has_next()) {
            auto generator = f(next());
            while (generator.has_next()) {
                co_yield generator.next();
    ...
    template <typename F> Generator take_while(F f) {
        while (has_next()) {
            T value = next();
            if (f(value)) {
                co_yield value;
    ...
    template <typename F> Generator filter(F f) {
        while (has_next()) {
            T value = next();
            if (f(value)) {
                co_yield value;
    ...
    template <typename R, typename F> R fold(R initial, F f) {
        while (has_next()) {
...
int main() {
    ...
    std::cout << Generator<double>::from(1.0, 2.0, 3.0, 4, 5, 6.0f).fold(1, [](auto acc, auto i) {
        return acc * i;
    }) << std::endl;
    return 0;
}