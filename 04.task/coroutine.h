...
template <class _Tp, class = void> struct __coroutine_traits_sfinae {};

template <class _Tp>
struct __coroutine_traits_sfinae<
    _Tp, typename async_simple::detail::__void_t<typename _Tp::promise_type>::type> {
...
public:
    ...
    bool done() const {
        assert(__is_suspended() && "done() can be called only on suspended coroutines");
    ...
    void resume() const {
        assert(__is_suspended() && "resume() can be called only on suspended coroutines");
        assert(!done() && "resume() has undefined behavior when the coroutine is done");
        __builtin_coro_resume(__handle_);
    }

    void destroy() const {
        assert(__is_suspended() && "destroy() can be called only on suspended coroutines");
...
template <typename _Promise> class coroutine_handle {
public:
    // [coroutine.handle.con], construct/reset
    constexpr coroutine_handle() noexcept = default;
    ...
    static coroutine_handle from_promise(_Promise& __promise) {
        ...
        __tmp.__handle_ = __builtin_coro_promise(
            std::addressof(const_cast<_RawPromise&>(__promise)), alignof(_Promise), true);
        return __tmp;
    }
...
template <class _Tp> struct hash<STD_CORO::coroutine_handle<_Tp>> {
